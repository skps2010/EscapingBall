...
public class play extends JFrame {
	...
	int people = 2;
	boolean mp = false;
	char[][] levelColor = new char[][] {
			"0202002020300000000400000000003000000004000000000000000000003000000004000000000030000000040101001010"
					.toCharArray(),
			"0202020202030000000004000000000003000000000400000000000300000000040000000000030000000004000000000003000000000401010101010"
					.toCharArray() };
	Color[] color = new Color[] { Color.blue, new Color(0x009100), new Color(0xFF8000), new Color(0x6F00D2),
			new Color(0x0000E3) };
	String[] stage = {
			"0101001010100000000100000000001000000001000033000000003300001000000001000000000010000000010101001010",
	...
	Level[] playLevel = new Level[] { new Level("第一關", 10, 10, stage[0]), new Level("第二關", 11, 11, stage[1]) };
	...
	public void set() {
		...
		addKeyListener(new KeyListener() {
			...
			public void keyPressed(KeyEvent e) {
				...
				if (gx != -1 || replay != 0)
					return;
				if (mp && stageColor[page][focus] != (int) (step / 2) % (people + 2) + 49)
					return;
		...
		addMouseListener(new MouseListener() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if (state == State.menu) {
					...
					if (mx > 180 && mx < 460 && my > 460 && my < 520)
						c = new create();
					if (mx > 180 && mx < 460 && my > 540 && my < 600)
						state = State.info;
				}

				if (state == State.select) {
					if (mx > 270 && mx < 370 && my > 590 && my < 630)
						state = State.menu;
					if (levelList.size() == 0)
						return;
					if (mx > 30 && mx < 120 && my > 540 && my < 580 && page != 0) {
						page--;
						if (mp)
							changePeople();
					}
					if (mx > 520 && mx < 610 && my > 540 && my < 580 && page != levelList.size() - 1) {
						page++;
						if (mp)
							changePeople();
					}
					...
					if (mp) {
						...
						if (mx > 15 && mx < 95 && my > 140 && my < 190) {
							people = 0;
							changePeople();
						}
						if (mx > 15 && mx < 950 && my > 220 && my < 270) {
							people = 1;
							changePeople();
						}
						if (mx > 15 && mx < 95 && my > 300 && my < 350) {
							people = 2;
							changePeople();
						}
				...
				if (state == State.game) {
					...
					if (mx > 10 && mx < 60 && my > 400 && my < 480 && focus != lastStart && step != 0)// 復原
					{
						if (gx == -1) {
							level[lastGoal]--;
							if (mp)
								stageColor[page][lastGoal] = '0';
						}
						gx = -1;
						replay = 0;
						level[lastStart]++;
						if (mp)
							stageColor[page][lastStart] = ballColor;
						focus = lastStart;
						step--;
					}
					...
					if (mx > xo && mx < 640 - xo && my > yo && my < 640 - yo && replay == 0)// 遊戲
					{
						...
						if (level[f] == '1' || level[f] == '4')
							...
							if (mx - a > Math.abs(my - b))
								move = 1;
							else if (mx - a < -Math.abs(my - b))
								move = -1;
							else if (my - b > 0)
								move = x;
							else
	...
	public void changePeople() {
		...
		for (int i = 0; i < 2; i++) {
			...
			if (page == 0) {
				...
				for (int j = 0; j < 5; j++)
					levelList.get(1).level[add[page * 2 + i] + j * 22] = a;
				for (int j = 0; j < 5; j++)
					stageColor[1][add[page * 2 + i] + j * 22] = b;
			}
		}
	}
	...
	public boolean detect(int i, int m) {
		if (i == -1)
			return false;
		if (m == -x && i >= x)
			if (level[i - x] == '0' || level[i - x] == '3')
				return true;
		if (m == 1 && i % x != x - 1)
			if (level[i + 1] == '0' || level[i + 1] == '3')
				return true;
		if (m == x && i < x * y - x)
			if (level[i + x] == '0' || level[i + x] == '3')
				return true;
	...
	public void ani() {
		...
		if (gx % 50 == 0 && gy % 50 == 0) {
			if (detect(nextFocus, dir))
				...
				if (replay >= history.length && replay != 0)
					replay = -1;
				if (replay > 0) {
	...
	public void saveRecord() {
		...
		try {
			...
			if (score(page) == 999999) {
				...
				out = new BufferedWriter(new FileWriter("Level/紀錄.txt", true));
				for (Step s : stepRecord) {
					if (s.name.equals(levelList.get(page).name))
						s.step = step;
	...
	public void paint(Graphics paint) {
		...
		if (state == State.select) {
			...
			if (levelList.size() == 0) {
				...
				if (!mp) {
					if (score(page) == 999999)
						g.drawString("最佳紀錄:無", 10, 620);
					else
						g.drawString("最佳紀錄:" + score(page), 10, 620);
				} else {
		...
		if (state == State.game) {
			...
			if (!mp) {
				if (ing)
					g.setColor(new Color(0xCC0080));
				else
					g.setColor(new Color(0x73B839));
			...
			if (!mp)
				if (score(page) == 999999)
					g.drawString("最佳紀錄:無", 150, 620);
				else
					g.drawString("最佳紀錄:" + score(page), 150, 620);
			else
				g.drawString(words[(int) (step / 2) % (people + 2) + 1] + "方還剩" + (2 - step % 2) + "步", 150, 620);
			g.drawString(step + "", 430, 620);
			if (msg != 0)
				g.drawString("無解", 280, 60);
			if (ing)
				g.drawString("計算中：第" + turn + "步", 180, 60);
			g.setColor(Color.yellow);
			g.setFont(new Font(h, Font.PLAIN, 30));
			g.drawString("返回", 33, 620);
